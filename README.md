...
- Also comes with various useful features for debugging
  ...
  - **Print var:** Automated insertion of print statement to print a variable
    ...
    - Using this function in visual mode will print out whatever is in the
      visual selection.
    - Setting `print_var_normal_mode = true` in setup, will automatically find the variable
      under the cursor and print it in *normal mode* instead of visual.
...
require('refactoring').setup({
    ...
    -- for `print_var`, will us what's in visual mode to print by default
    -- if set `true` will attemp to find variable under curosr in normal mode
